...
{{-- === PANDUAN KONTEKSTUAL ATUR THRESHOLD === --}}
@section('help-content')
    <div class="help-alert">
        <i class="fa fa-info-circle"></i> 
        <strong>KONSEP DASAR:</strong> Halaman ini digunakan untuk mengatur batas <strong>Fast Moving</strong> dan <strong>Slow Moving</strong> pada setiap barang per kategori.
    </div>

    <h4 class="help-h"><i class="fa fa-shield text-primary"></i> Fungsi & Tujuan</h4>
    <p class="help-p">
        Fitur ini berfungsi untuk menentukan klasifikasi perputaran barang (Threshold) guna keperluan analisis stok dan logistik.
    </p>

    <h4 class="help-h"><i class="fa fa-magic text-primary"></i> Cara Penggunaan Cepat (Power User)</h4>
    <p class="help-p">Gunakan fitur pintar berikut untuk mempercepat pengisian:</p>
    <ul class="help-list">
        <li>
            <strong>Isi Massal per Barang (Baris):</strong> 
            Klik ikon <i class="fa fa-pencil-square-o text-primary"></i> di samping <strong>Nama Barang</strong> untuk mengisi angka Batas Fast dan Batas Slow yang sama pada barang tersebut sekaligus.
        </li>
        <li>
            <strong>Isi Massal per Kolom:</strong> 
            Klik ikon <i class="fa fa-pencil-square-o text-primary"></i> di bawah header <strong>Batas Fast</strong> atau <strong>Batas Slow</strong> untuk menyamakan semua nilai kolom tersebut untuk seluruh barang yang tampil.
        </li>
    </ul>
...
<style>
    ...
    /* Header Styling */
    .matrix-table th {
        background: var(--table-head); padding: 12px 10px; 
...
<div class="app-container">
    ...
    <div class="pg-header">
        <div class="pg-title">
            ...
            <div class="pg-text-wrap">
                <h1>Atur Threshold</h1>
                <div class="pg-subtitle">Kelola Batas Fast Moving & Batas Slow Moving Barang</div>
            </div>
        </div>
    </div>
    ...
    <div class="cat-tabs">
        @foreach($categories as $cat)
            ...
            <a href="{{ route('thresholds.index', ['cat_id' => $cat->id]) }}" 
                        ...
                        <tr>
                            ...
                            <th style="min-width: 150px;">
                                <div style="display:flex; flex-direction:column; align-items:center; gap:4px;">
                                    <span>BATAS FAST</span>
                                    <i class="fa fa-pencil-square-o mass-btn" 
                                       title="Isi Massal Batas Fast" 
                                       onclick="massEditColumn('fast', 'Batas Fast')"></i>
                                </div>
                            </th>
                            <th style="min-width: 150px;">
                                <div style="display:flex; flex-direction:column; align-items:center; gap:4px;">
                                    <span>BATAS SLOW</span>
                                    <i class="fa fa-pencil-square-o mass-btn" 
                                       title="Isi Massal Batas Slow" 
                                       onclick="massEditColumn('slow', 'Batas Slow')"></i>
                                </div>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($items as $item)
                            <tr class="item-row">
                                ...
                                <td>
                                    <input type="number" 
                                           name="threshold[{{ $item->id }}][batas_fast_moving]" 
                                           value="{{ $item->batas_fast_moving == 0 ? '0' : $item->batas_fast_moving }}" 
                                           class="cell-input col-fast row-{{ $item->id }}" 
                                           min="0"
                                ...
                                <td>
                                    <input type="number" 
                                           name="threshold[{{ $item->id }}][batas_slow_moving]" 
                                           value="{{ $item->batas_slow_moving == 0 ? '0' : $item->batas_slow_moving }}" 
                                           class="cell-input col-slow row-{{ $item->id }}" 
                                           min="0"
                                           onfocus="if(this.value=='0'){this.value=''}" 
                                           onblur="if(this.value==''){this.value='0'}" 
                                           onchange="markChanged(this)">
...
<script>
    ...
    document.getElementById('tableSearch').addEventListener('keyup', function() {
        ...
        var rows = document.querySelectorAll('#thresholdTable tbody tr');
    ...
    // 6. Mass Edit Column
    window.massEditColumn = function(type, typeLabel) {
        Swal.fire({
            title: '<i class="fa fa-pencil-square-o"></i> Isi Otomatis',
            html: `Masukkan nilai threshold untuk semua item<br>pada kolom <strong style="color:#4F46E5">${typeLabel}</strong>`,
            input: 'number',
            ...
            if (result.isConfirmed && result.value !== "") {
                ...
                var inputs = document.querySelectorAll('.col-' + type);
                var count = 0;
    ...
    window.massEditRow = function(itemId, itemName) {
        Swal.fire({
            title: '<i class="fa fa-pencil-square-o"></i> Isi Otomatis',
            html: `Masukkan nilai threshold untuk barang<br><strong style="color:#4F46E5">${itemName}</strong><br>untuk Batas Fast & Batas Slow.`,
            input: 'number',
            inputAttributes: { min: 0, autofocus: 'true' },
            inputPlaceholder: '',
            backdrop: `rgba(0,0,0,0.5)`,
            ...
            if (result.isConfirmed && result.value !== "") {
                ...
                isDirty = true;
                const Toast = Swal.mixin({toast: true, position: 'bottom-end', showConfirmButton: false, timer: 3000, timerProgressBar: true});
                Toast.fire({icon: 'success', title: 'Kedua threshold diperbarui.'});
            }
        });
    }
</script>
